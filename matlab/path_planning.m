%% Lecture 8 — Task-space path planning narrative
% This script ties together interpolation and iterative IK to realise a
% pick-and-place style motion. Run sections sequentially.

clearvars;
clc;

%% 1. Load robot model and initial configuration
fprintf('\n=== Step 1: Robot model ===\n');
[dh_table, q_limits] = load_robot_dh('3R_planar');
disp(dh_table);
q0 = [0, -20, 40] * pi/180;
fprintf('Initial joint vector (deg): [%.1f %.1f %.1f]\n', q0 * 180/pi);

%% 2. Define start/goal poses and interpolate waypoints
fprintf('\n=== Step 2: Interpolate task-space waypoints ===\n');
T_start = transl(0.35, 0.05, 0.10);
T_pick  = transl(0.45, 0.00, 0.08) * rotZ(30*pi/180);
T_place = transl(0.55, -0.10, 0.12) * rotZ(80*pi/180);
segment1 = interpolate_pose(T_start, T_pick, 5);
segment2 = interpolate_pose(T_pick, T_place, 7);
waypoints = [segment1; segment2(2:end)];
fprintf('Generated %d waypoints.\n', numel(waypoints));

%% 3. Follow the path via damped least-squares IK
fprintf('\n=== Step 3: Follow path ===\n');
q_traj = follow_path(dh_table, q0, waypoints, 0.07);
fprintf('Resulting trajectory shape: %d×%d\n', size(q_traj,1), size(q_traj,2));

%% 4. Inspect manipulability along the path
fprintf('\n=== Step 4: Manipulability along trajectory ===\n');
for k = 1:size(q_traj,1)
    J = jacobian_geometric(dh_table, q_traj(k,:));
    w = manipulability(J);
    fprintf('Waypoint %02d → manipulability %.4f\n', k, w);
end

%% Local helper functions (self-contained copy)
...
for k = 1:numel(waypoints)
    q = ik_to_pose(dh_table, q, waypoints{k}, lambda);
    q_traj(k,:) = q;
end
end
...
warning('    IK step hit iteration limit with residual %.2e.', norm(err));
end
